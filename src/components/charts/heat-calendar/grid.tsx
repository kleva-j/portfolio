    ...
    >
      ...
      {cols.map(({ id, w }) =>
        DAYS.map(({ id: dayId, d }) => {
          ...
          const isEnd = span
            ? i === span.lo || i === span.hi
            : pinned?.w === w && pinned?.d === d;
          const dim =
            (step !== null && step !== b) ||
            (span !== null && (i < span.lo || i > span.hi));
          // the ripple: the hovered cell rises most, the ring around it a little, two out barely
          const dist = hot
            ? Math.max(Math.abs(hot.w - w), Math.abs(hot.d - d))
            : 9;
          const lift =
            reduce || !canHover
              ? 1
          ...
          return (
            ...
            >
              ...
              <motion.button
                ...
                whileTap={
                  reduce ? undefined : { scale: 0.9, transition: SPRING_PRESS }
                }
              >
                <motion.span
