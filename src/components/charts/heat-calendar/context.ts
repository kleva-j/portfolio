...
export function useHeatCalendarModel({
  ...
  const end = useMemo(
    () => (endDate ? startOfDay(endDate) : today),
    [endDate, today],
  );
  const start = useMemo(
  ...
  const level = (w: number, d: number) =>
    Math.max(0, Math.min(1, values?.[w]?.[d] ?? 0));
  ...
  const dateOf = (w: number, d: number) =>
    start ? addDays(start, w * 7 + d) : null;
  ...
  const cols = useMemo(() => {
    const list = Array.from({ length: weeks }, (_, w) => {
      ...
      return {
        ...
        w,
        m,
        label: fresh && date ? fmtMonth.format(date) : null,
      };
    });
  ...
  const span =
    pinned && spanTo
      ? {
          lo: Math.min(idx(pinned), idx(spanTo)),
          hi: Math.max(idx(pinned), idx(spanTo)),
        }
      : null;
  let spanTotal = 0;
  if (span) {
...
export const HeatCalendarContext = createContext<ReturnType<
  typeof useHeatCalendarModel
> | null>(null);
...
export function useHeatCalendar() {
  ...
  if (!context)
    throw new Error("HeatCalendar parts must be inside HeatCalendar");
  return context;
}
